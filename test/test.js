...
contract('Decentragram', ([deployer, author, tipper]) => {
  ...
  describe('images', async () => {
    ...
    it('creates images', async () => {
      ...
      assert.equal(event.id.toNumber(), imageCount, "Id is not correct")
      assert.equal(event.hash, hash, "Hash is not correct")
      assert.equal(event.description, description, "Description is not correct")
      assert.equal(event.tipAmount.toNumber(), 0, "Tip amount is not correct")
    ...
    it('list images', async () => {
      ...
      assert.equal(image.id.toNumber(), imageCount, "Id is not correct")
      assert.equal(image.hash, hash, "Hash is not correct")
      assert.equal(image.description, description, "Description is not correct")
      assert.equal(image.tipAmount.toNumber(), 0, "Tip amount is not correct")
    ...
    it('allows users to tip images', async () => {
      ...
      assert.equal(event.hash, hash, "Hash is not correct")
      assert.equal(event.description, description, "Description is not correct")
      assert.equal(event.tipAmount, web3.utils.toWei('1', 'ether').toString(), "Tip amount is not correct")
      assert.equal(event.author, author, "Author is not correct")
      ...
      assert.equal(newAuthorBalance.toString(), expectedBalance.toString(), '123')

      await decentragram.tipImageOwner(
          99,
          { from: tipper, value: web3.utils.toWei('1', 'ether')}
      ).should.be.rejected;
    })
